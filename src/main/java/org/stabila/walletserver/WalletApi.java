package org.stabila.walletserver;
...
import org.stabila.api.GrpcAPI.TransactionSignWeight.Result.response_code;
...
import org.stabila.keystore.CheckStrength;
...
import org.stabila.protos.contract.StorageContract.BuyStorageBytesContract;
...
public class WalletApi {
  ...
  public static GrpcClient init() {
    ...
    if (config.hasPath("net.type") && "mainnet".equalsIgnoreCase(config.getString("net.type"))) {
      ...
      DecodeUtil.addressPreFixByte = CommonConstant.ADD_PRE_FIX_BYTE_TESTNET;
    }
  ...
  private Transaction signTransaction(Transaction transaction)
    ...
    while (true) {
      ...
      byte[] passwd = org.stabila.keystore.StringUtils.char2Byte(password);
      org.stabila.keystore.StringUtils.clear(password);
      if (isEckey) {
      ...
      org.stabila.keystore.StringUtils.clear(passwd);
  ...
  public boolean createExecutive(byte[] owner, byte[] url)
      throws CipherException, IOException, CancelException {
    if (owner == null) {
      owner = getAddress();
    }

    ExecutiveCreateContract contract = createExecutiveCreateContract(owner, url);
    if (rpcVersion == 2) {
      TransactionExtention transactionExtention = rpcCli.createExecutive2(contract);
      return processTransactionExtention(transactionExtention);
    } else {
      Transaction transaction = rpcCli.createExecutive(contract);
      return processTransaction(transaction);
    }
  }

  public boolean updateExecutive(byte[] owner, byte[] url)
      throws CipherException, IOException, CancelException {
    if (owner == null) {
      owner = getAddress();
    }

    ExecutiveUpdateContract contract = createExecutiveUpdateContract(owner, url);
    if (rpcVersion == 2) {
      TransactionExtention transactionExtention = rpcCli.updateExecutive2(contract);
      return processTransactionExtention(transactionExtention);
    } else {
      Transaction transaction = rpcCli.updateExecutive(contract);
      return processTransaction(transaction);
    }
  }
  ...
  public boolean voteExecutive(byte[] owner, HashMap<String, String> executive)
      throws CipherException, IOException, CancelException {
    if (owner == null) {
      owner = getAddress();
    }

    VoteExecutiveContract contract = createVoteExecutiveContract(owner, executive);
    if (rpcVersion == 2) {
      TransactionExtention transactionExtention = rpcCli.voteExecutiveAccount2(contract);
      return processTransactionExtention(transactionExtention);
    } else {
  ...
  public static ExecutiveCreateContract createExecutiveCreateContract(byte[] owner, byte[] url) {
  ...
  public static ExecutiveUpdateContract createExecutiveUpdateContract(byte[] owner, byte[] url) {
  ...
  public static VoteExecutiveContract createVoteExecutiveContract(byte[] owner,
  ...
  public boolean cdBalance(
      ...
      throws CipherException, IOException, CancelException {
    CdBalanceContract contract =
        createCdBalanceContract(
            ownerAddress, cded_balance, cded_duration, resourceCode, receiverAddress);
    if (rpcVersion == 2) {
      TransactionExtention transactionExtention = rpcCli.createTransaction2(contract);
      return processTransactionExtention(transactionExtention);
    } else {
  ...
  private CdBalanceContract createCdBalanceContract(
      byte[] address,
      long cded_balance,
      long cded_duration,
      int resourceCode,
    ...
    builder
        ...
        .setCdedBalance(cded_balance)
        .setCdedDuration(cded_duration)
  ...
  public boolean uncdBalance(byte[] ownerAddress, int resourceCode, byte[] receiverAddress)
      throws CipherException, IOException, CancelException {
    UncdBalanceContract contract =
        createUncdBalanceContract(ownerAddress, resourceCode, receiverAddress);
    if (rpcVersion == 2) {
      TransactionExtention transactionExtention = rpcCli.createTransaction2(contract);
      return processTransactionExtention(transactionExtention);
    } else {
  ...
  private UncdBalanceContract createUncdBalanceContract(
    ...
    UncdBalanceContract.Builder builder =
  ...
  public boolean uncdAsset(byte[] ownerAddress)
      throws CipherException, IOException, CancelException {
    UncdAssetContract contract = createUncdAssetContract(ownerAddress);
    if (rpcVersion == 2) {
      TransactionExtention transactionExtention = rpcCli.createTransaction2(contract);
      return processTransactionExtention(transactionExtention);
    } else {
  ...
  private UncdAssetContract createUncdAssetContract(byte[] address) {
    if (address == null) {
      address = getAddress();
    }
  ...
  public static UpdateUcrLimitContract createUpdateUcrLimitContract(
      byte[] owner, byte[] contractAddress, long originUcrLimit) {
  ...
  public static CreateSmartContract createContractDeployContract(
    ...
    builder
        ...
        .setOriginUcrLimit(originUcrLimit);
  ...
  public byte[] generateContractAddress(byte[] ownerAddress, Transaction stb) {
    // get tx hash
    byte[] txRawDataHash = Sha256Sm3Hash.of(stb.getRawData().toByteArray()).getBytes();
  ...
  public boolean updateUcrLimit(byte[] owner, byte[] contractAddress, long originUcrLimit)
      throws IOException, CipherException, CancelException {
    if (owner == null) {
      owner = getAddress();
    }

    UpdateUcrLimitContract updateUcrLimitContract = createUpdateUcrLimitContract(owner,
        contractAddress, originUcrLimit);

    TransactionExtention transactionExtention = rpcCli.updateUcrLimit(updateUcrLimitContract);
    if (transactionExtention == null || !transactionExtention.getResult().getResult()) {
      System.out.println("RPC create stb failed!");
      if (transactionExtention != null) {
        System.out.println("Code = " + transactionExtention.getResult().getCode());
  ...
  public boolean triggerContract(
    ...
    if (transaction.getRetCount() != 0
      ...
      System.out.println("UcrUsed:" + transactionExtention.getUcrUsed());
      return true;
    }
  ...
  public AccountPermissionUpdateContract createAccountPermissionContract(byte[] owner,
    ...
    JSONObject executive_permission = permissions.getJSONObject("executive_permission");
    ...
    if (executive_permission != null) {
      Permission executivePermission = json2Permission(executive_permission);
  ...
  public static boolean sendShieldedCoinWithoutAsk(PrivateParametersWithoutAsk privateParameters,
    ...
    BytesMessage stbHash = rpcCli.getShieldedTransactionHash(transactionExtention.getTransaction());
    ...
    for (int i = 0; i < spendDescList.size(); i++) {
      ...
      builder.setTxHash(ByteString.copyFrom(stbHash.getValue().toByteArray()));
