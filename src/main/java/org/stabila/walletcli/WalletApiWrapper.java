...
//      System.out.println("Warning: GetAddress failed,  Please login first !!");
  ...
  public boolean assetIssue(byte[] ownerAddress, String name, String abbrName, long totalSupply,
      int stbNum, int icoNum,
      int precision, long startTime, long endTime, int voteScore, String description, String url,
      long freeNetLimit, long publicFreeNetLimit, HashMap<String, String> cdedSupply)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
    ...
    if (stbNum <= 0) {
      System.out.println("stbNum should greater than 0. but really is " + stbNum);
      return false;
    }
    ...
    for (String daysStr : cdedSupply.keySet()) {
      ...
      AssetIssueContract.CdedSupply.Builder cdedSupplyBuilder
          = AssetIssueContract.CdedSupply.newBuilder();
      cdedSupplyBuilder.setCdedAmount(amount);
      cdedSupplyBuilder.setCdedDays(days);
      builder.addCdedSupply(cdedSupplyBuilder.build());
    }
  ...
  public boolean createExecutive(byte[] ownerAddress, String url)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: createExecutive failed,  Please login first !!");
      return false;
    }

    return wallet.createExecutive(ownerAddress, url.getBytes());
  }

  public boolean updateExecutive(byte[] ownerAddress, String url)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: updateExecutive failed,  Please login first !!");
      return false;
    }

    return wallet.updateExecutive(ownerAddress, url.getBytes());
  }
  ...
  public boolean voteExecutive(byte[] ownerAddress, HashMap<String, String> executive)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: VoteExecutive failed,  Please login first !!");
      return false;
    }

    return wallet.voteExecutive(ownerAddress, executive);
  }

  public Optional<ExecutiveList> listExecutives() {
    try {
      return WalletApi.listExecutives();
    } catch (Exception ex) {
      ex.printStackTrace();
  ...
  public boolean cdBalance(byte[] ownerAddress, long cded_balance, long cded_duration,
      int resourceCode, byte[] receiverAddress)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: cdBalance failed, Please login first !!");
      return false;
    }

    return wallet.cdBalance(ownerAddress, cded_balance, cded_duration, resourceCode,
        receiverAddress);
  }
  ...
  public boolean uncdBalance(byte[] ownerAddress, int resourceCode, byte[] receiverAddress)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: uncdBalance failed, Please login first !!");
      return false;
    }

    return wallet.uncdBalance(ownerAddress, resourceCode, receiverAddress);
  }
  ...
  public boolean uncdAsset(byte[] ownerAddress)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: uncdAsset failed, Please login first !!");
      return false;
    }

    return wallet.uncdAsset(ownerAddress);
  }
  ...
  public boolean updateUcrLimit(byte[] ownerAddress, byte[] contractAddress,
      long originUcrLimit)
      throws CipherException, IOException, CancelException {
    if (wallet == null || !wallet.isLoginState()) {
      System.out.println("Warning: updateSetting failed,  Please login first !!");
    ...
    return wallet.updateUcrLimit(ownerAddress, contractAddress, originUcrLimit);
  }
  ...
  public boolean deployContract(byte[] ownerAddress, String name, String abiStr, String codeStr,
      long feeLimit, long value, long consumeUserResourcePercent, long originUcrLimit,
    ...
    return wallet
        .deployContract(ownerAddress, name, abiStr, codeStr, feeLimit, value,
            ...
            originUcrLimit, tokenValue, tokenId,
