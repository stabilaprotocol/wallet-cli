...
import org.stabila.core.zen.ShieldedAddressInfo;
import org.stabila.core.zen.ShieldedNoteInfo;
import org.stabila.core.zen.ShieldedTRC20NoteInfo;
import org.stabila.core.zen.ShieldedTRC20Wrapper;
import org.stabila.core.zen.ShieldedWrapper;
import org.stabila.core.zen.ZenUtils;
...
import org.stabila.keystore.StringUtils;
...
public class Client {
  ...
  private static String[] commandHelp = {
      ...
      "CreateExecutive",
      "DeleteProposal",
      "DeployContract contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent origin_ucr_limit value token_value token_id <library:address,library:address,...> <lib_compiler_version(e.g:v5)>",
      "ExchangeCreate",
      "ExchangeInject",
      "ExchangeTransaction",
      "ExchangeWithdraw",
  ...
  private void sendCoin(String[] parameters) throws IOException, CipherException, CancelException {
    ...
    if (result) {
      System.out.println("Send " + amount + " Unit to " + base58ToAddress + " successful !!");
    } else {
      System.out.println("Send " + amount + " Unit to " + base58ToAddress + " failed !!");
    }
  }
  ...
  private void assetIssue(String[] parameters)
    ...
    if (parameters == null || parameters.length < 12) {
      ...
      System.out.println("AssetIssue [OwnerAddress] AssetName AbbrName TotalSupply StbNum AssetNum Precision "
              + "StartDate EndDate Description Url FreeNetLimitPerAccount PublicFreeNetLimit "
              + "CdedAmount0 CdedDays0 ... CdedAmountN CdedDaysN");
      System.out.println("StbNum and AssetNum represents the conversion ratio of the stabila to the asset.");
    ...
    String stbNumStr = parameters[index++];
    ...
    boolean result = walletApiWrapper.assetIssue(ownerAddress, name, abbrName, totalSupply,
        stbNum, icoNum, precision, startTime, endTime, 0,
        description, url, freeAssetNetLimit, publicFreeNetLimit, cdedSupply);
    if (result) {
  ...
  private void createExecutive(String[] parameters)
      throws IOException, CipherException, CancelException {
    if (parameters == null || (parameters.length != 1 && parameters.length != 2)) {
      System.out.println("CreateExecutive needs 1 parameter using the following syntax: ");
      System.out.println("CreateExecutive [OwnerAddress] Url");
      return;
    }
    ...
    boolean result = walletApiWrapper.createExecutive(ownerAddress, url);
    if (result) {
      System.out.println("CreateExecutive successful !!");
    } else {
      System.out.println("CreateExecutive failed !!");
    }
  }

  private void updateExecutive(String[] parameters)
      throws IOException, CipherException, CancelException {
    if (parameters == null || (parameters.length != 1 && parameters.length != 2)) {
      System.out.println("updateExecutive needs 1 parameter using the following syntax: ");
      System.out.println("updateExecutive [OwnerAddress] Url");
      return;
    }
    ...
    boolean result = walletApiWrapper.updateExecutive(ownerAddress, url);
    if (result) {
      System.out.println("updateExecutive successful !!");
    } else {
      System.out.println("updateExecutive failed !!");
    }
  }

  private void listExecutives() {
    Optional<ExecutiveList> result = walletApiWrapper.listExecutives();
    if (result.isPresent()) {
      ExecutiveList executiveList = result.get();
      System.out.println(Utils.formatMessageString(executiveList));
    } else {
      System.out.println("List executives failed !!");
    }
  }
  ...
  private void voteExecutive(String[] parameters)
      throws IOException, CipherException, CancelException {
    if (parameters == null || parameters.length < 2) {
      System.out.println("Use VoteExecutive command with below syntax: ");
      System.out.println("VoteExecutive [OwnerAddress] Address0 Count0 ... AddressN CountN");
      return;
    }
    ...
    HashMap<String, String> executive = new HashMap<String, String>();
    while (index < parameters.length) {
      String address = parameters[index++];
      String countStr = parameters[index++];
      executive.put(address, countStr);
    }

    boolean result = walletApiWrapper.voteExecutive(ownerAddress, executive);
    if (result) {
      System.out.println("VoteExecutive successful !!!");
    } else {
      System.out.println("VoteExecutive failed !!!");
    }
  }
  ...
  private void cdBalance(String[] parameters)
      throws IOException, CipherException, CancelException {
    if (parameters == null || !(parameters.length == 2 || parameters.length == 3
        || parameters.length == 4 || parameters.length == 5)) {
      System.out.println("Use cdBalance command with below syntax: ");
      System.out.println("cdBalance [OwnerAddress] cded_balance cded_duration "
          + "[ResourceCode:0 BANDWIDTH,1 UCR,2 STABILA_POWER] [receiverAddress]");
      return;
    }
    ...
    long cded_balance = Long.parseLong(parameters[index++]);
    long cded_duration = Long.parseLong(parameters[index++]);
    int resourceCode = 0;
    ...
    boolean result = walletApiWrapper.cdBalance(ownerAddress, cded_balance,
        cded_duration, resourceCode, receiverAddress);
    if (result) {
      System.out.println("CdBalance successful !!!");
    } else {
      System.out.println("CdBalance failed !!!");
    }
  }

  private void uncdBalance(String[] parameters)
      throws IOException, CipherException, CancelException {
    if (parameters == null || parameters.length < 1 || parameters.length > 3) {
      System.out.println("Use uncdBalance command with below syntax: ");
      System.out.println(
          "uncdBalance [OwnerAddress] ResourceCode(0 BANDWIDTH,1 UCR,2 STABILA_POWER) [receiverAddress]");
      return;
    }
    ...
    boolean result = walletApiWrapper.uncdBalance(ownerAddress, resourceCode, receiverAddress);
    if (result) {
      System.out.println("UncdBalance successful !!!");
    } else {
      System.out.println("UncdBalance failed !!!");
    }
  }

  private void uncdAsset(String[] parameters) throws IOException,
      CipherException, CancelException {
    System.out.println("Use UncdAsset command like: ");
    System.out.println("UncdAsset [OwnerAddress] ");
    ...
    boolean result = walletApiWrapper.uncdAsset(ownerAddress);
    if (result) {
      System.out.println("UncdAsset successful !!!");
    } else {
      System.out.println("UncdAsset failed !!!");
    }
  }
...
//  }
  ...
  private void updateUcrLimit(String[] parameters)
      throws IOException, CipherException, CancelException {
    if (parameters == null || (parameters.length != 2 && parameters.length != 3)) {
      System.out.println("Using updateUcrLimit command needs 2 parameters like: ");
      System.out.println("updateUcrLimit [OwnerAddress] contract_address ucr_limit");
      return;
    }
    ...
    long originUcrLimit = Long.valueOf(parameters[index++]).longValue();
    if (originUcrLimit < 0) {
      System.out.println("origin_ucr_limit need > 0 ");
      return;
    }
    boolean result = walletApiWrapper
        .updateUcrLimit(ownerAddress, contractAddress, originUcrLimit);
    if (result) {
      System.out.println("UpdateSetting for origin_ucr_limit successful !!!");
    } else {
      System.out.println("UpdateSetting for origin_ucr_limit failed !!!");
    }
  }
  ...
  private void deployContract(String[] parameter)
    ...
    if (parameters == null ||
      ...
      System.out.println(
          "DeployContract [ownerAddress] contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent origin_ucr_limit value token_value token_id(e.g: STBTOKEN, use # if don't provided) <library:address,library:address,...> <lib_compiler_version(e.g:v5)>");
//      System.out.println(
//          "Note: Please append the param for constructor tightly with byteCode without any space");
    ...
    long originUcrLimit = Long.parseLong(parameters[idx++]);
    if (consumeUserResourcePercent > 100 || consumeUserResourcePercent < 0) {
      System.out.println("consume_user_resource_percent should be >= 0 and <= 100");
      return;
    }
    if (originUcrLimit <= 0) {
      System.out.println("origin_ucr_limit must > 0");
      return;
    }
    ...
    boolean result = walletApiWrapper
        .deployContract(ownerAddress, contractName, abiStr, codeStr, feeLimit, value,
            consumeUserResourcePercent, originUcrLimit, tokenValue, tokenId, libraryAddressPair,
            compilerVersion);
    if (result) {
  ...
  private void triggerContract(String[] parameters, boolean isConstant)
    ...
    if (isConstant) {
      ...
      if (parameters == null || (parameters.length != 8 && parameters.length != 9)) {
        ...
        System.out.println(cmdMethodStr + " [OwnerAddress] contractAddress method args isHex"
            + " fee_limit value token_value token_id(e.g: STBTOKEN, use # if don't provided)");
        return;
      }
    }
  ...
  private void getSpendingKey() {
    while (true) {
      byte[] skBytes = Wallet.generateRandomBytes(32);
  ...
  private void help() {
    System.out.println("Help: List of Stabila Wallet-cli commands");
    System.out.println(
  ...
  private void run() {
    System.out.println(" ");
    System.out.println("Welcome to Stabila Wallet-Cli");
        ...
        try {
          ...
          switch (cmdLowerCase) {
            ...
            case "createexecutive": {
              createExecutive(parameters);
              break;
            }
            case "updateexecutive": {
              updateExecutive(parameters);
              break;
            }
            case "voteexecutive": {
              voteExecutive(parameters);
              break;
            }
            case "cdbalance": {
              cdBalance(parameters);
              break;
            }
            case "uncdbalance": {
              uncdBalance(parameters);
              break;
            }
